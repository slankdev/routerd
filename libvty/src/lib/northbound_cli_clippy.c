...
/* config_load => "configuration load          <	    file [<json$json|xml$xml> [translate WORD$translator_family]] FILENAME$filename	    |transaction (1-4294967296)$tid	  >	  [replace$replace]" */
...
/* show_config_compare => "show configuration compare          <	    candidate$c1_candidate	    |running$c1_running	    |transaction (1-4294967296)$c1_tid	  >          <	    candidate$c2_candidate	    |running$c2_running	    |transaction (1-4294967296)$c2_tid	  >	  [<json$json|xml$xml> [translate WORD$translator_family]]" */
...
/* show_config_transaction => "show configuration transaction          [	    (1-4294967296)$transaction_id	    [<json$json|xml$xml> [translate WORD$translator_family]]            [<	      with-defaults$with_defaults	      |changes$changes	     >]	  ]" */
...
/* show_yang_operational_data => "show yang operational-data XPATH$xpath         [{	   format <json$json|xml$xml>	   |translate WORD$translator_family	 }]" */
...
DEFUN_CMD_FUNC_TEXT(show_yang_operational_data)
{
	...
	return show_yang_operational_data_magic(self, vty, argc, argv, xpath, json, xml, translator_family);
}
...
/* show_yang_module_detail => "show yang module          [module-translator WORD$translator_family]          WORD$module_name <summary|tree$tree|yang$yang|yin$yin>" */
...
DEFUN_CMD_FUNC_TEXT(show_yang_module_detail)
{
	...
	return show_yang_module_detail_magic(self, vty, argc, argv, translator_family, module_name, tree, yang, yin);
}
...
/* rollback_config => "rollback configuration (1-4294967296)$transaction_id" */
...
/* debug_nb => "[no] debug northbound          [<	    callbacks$cbs [{configuration$cbs_cfg|state$cbs_state|rpc$cbs_rpc}]	    |notifications$notifications	    |events$events          >]" */
...
DEFUN_CMD_FUNC_TEXT(debug_nb)
{
	...
	return debug_nb_magic(self, vty, argc, argv, no, cbs, cbs_cfg, cbs_state, cbs_rpc, notifications, events);
}
