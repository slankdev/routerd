...
main(int argc, const char **argv)
{
  ...
  for (size_t i=0; i<grps.size(); i++) {
    ...
    else if (grps[i] == "ipv4addr") group |= RTMGRP_IPV4_IFADDR;
    else if (grps[i] == "ipv6addr") group |= RTMGRP_IPV6_IFADDR;
    else if (grps[i] == "ipv4route") group |= RTMGRP_IPV4_ROUTE;
    else if (grps[i] == "ipv6route") group |= RTMGRP_IPV6_ROUTE;
  }
