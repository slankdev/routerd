...
int monitor_from_file(const std::string& filename)
{
  ...
  while (true) {
    ...
    rtnl_hexdump(NULL, NULL, (struct nlmsghdr*)buf, NULL);
  }
}
...
main(int argc, const char **argv)
{
  ...
  if (readfile != "") {
    ...
    int ret = netlink_listen(nl, rtnl_hexdump, NULL);
    if (ret < 0)
      return 1;
